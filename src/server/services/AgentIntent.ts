...
const REVISE_FIELD = '(?:GitHub|小红书|抖音|快手|B站|哔哩哔哩|微博|贴吧|知乎|百度|必应|360|搜狗|头条搜索|arXiv|论文库|AI\\s*(?:HOT|资讯搜索|行业资讯|新闻|圈动态|热点|热榜|日报)|DeepSeek|Kimi|豆包|千问|通义千问|Qwen|元宝|腾讯元宝|纳米AI|纳米 AI|文心|文心一言|文心言|文小言|BOSS\\s*直聘|zhipin\\.com|平台|关键词|评论|页|后台|分析目标|分析维度|关注重点)';
const REVISE = new RegExp(`(?:${REVISE_ACTION}.*${REVISE_FIELD}|${REVISE_FIELD}.*${REVISE_ACTION})`, 'i');
const RESEARCH = /采集|收集|抓取|搜索|搜(?:一下)?|查询|检索|查(?:找|一下)|调查|调研|研究|监测|做(?:个|一份)?报告|(?:我)?(?:想|要|想要)了解|帮我(?:查|搜|看看)|(?:网上|全网|各平台|社交媒体).*(?:口碑|评价|讨论|反馈|怎么说)|(?:看看|了解)(?:大家|网友|用户).*(?:评价|看法|反馈|怎么说)|(?:GitHub|AI\s*(?:HOT|资讯搜索|行业资讯|新闻|圈动态|热点|热榜|日报)).*(?:趋势|热门|仓库|项目|热点|日报|新闻|资讯|动态|消息)|(?:去|到|在)?(?:GitHub|小红书|抖音|快手|B站|哔哩哔哩|微博|百度贴吧|贴吧|知乎|百度|必应|360|搜狗|头条搜索|arXiv|论文库|AI\s*(?:HOT|资讯搜索|行业资讯|新闻|圈动态|热点|热榜|日报)|DeepSeek|Kimi|豆包|千问|通义千问|Qwen|元宝|腾讯元宝|纳米AI|纳米 AI|文心|文心一言|文心言|文小言|BOSS\s*直聘|(?:[\w-]+\.)*zhipin\.com)(?:上|里)?(?:搜|找|查|查询|检索|问|看看|读取)/i;
...
function cleanResearchSubject(text: string): string {
  ...
  return withoutBossPlatform
    ...
    .replace(/(?:不要|不采|不抓|不搜|排除|除去|除|移除|删除|去掉)(?:采集|抓取|搜索|查询)?\s*(?:黑猫投诉|黑猫|智联招聘|智联|前程无忧|51job|猎聘网|猎聘|arXiv|论文库|学术论文|学术文献|科研论文|AI\s*(?:HOT|资讯搜索|行业资讯|新闻|圈动态|热点|热榜|日报)|小红书|抖音|快手|B站|哔哩哔哩|微博|百度贴吧|贴吧|知乎|百度网页|百度搜索|百度|必应中国|必应|360搜索|360|搜狗搜索|搜狗|头条搜索|DeepSeek|Kimi(?:\s*AI)?|豆包|Doubao|通义千问|千问|Qwen|腾讯元宝|元宝|纳米\s*AI(?:搜索)?|文心一言|文心言|文小言|文心|平台)+/gi, ' ')
    .replace(/(?:黑猫投诉|黑猫|智联招聘|智联|前程无忧|51job|猎聘网|猎聘|arXiv|论文库|学术论文|学术文献|科研论文|AI\s*(?:HOT|资讯搜索|行业资讯|新闻|圈动态|热点|热榜|日报)|小红书|抖音|快手|B站|哔哩哔哩|微博|百度贴吧|贴吧|知乎|百度网页|百度搜索|百度|必应中国|必应|360搜索|360|搜狗搜索|搜狗|头条搜索|DeepSeek|Kimi(?:\s*AI)?|豆包|Doubao|通义千问|千问|Qwen|腾讯元宝|元宝|纳米\s*AI(?:搜索)?|文心一言|文心言|文小言|文心)(?:除外|不用|不要|不采|不抓|不搜)?/gi, ' ')
    .replace(/关键词(?:[:：]|\s)+/gi, ' ')
...
export function inferExcludedPlatforms(text: string): string[] {
  ...
  const aliases: Array<[RegExp, string]> = [
    ...
    [/(?:AI\s*(?:HOT|资讯搜索|行业资讯|新闻|圈动态|热点|热榜|日报)|aihot\.virxact\.com)/i, 'aihot'],
...
export function localIntentDecision(text: string, context: IntentContext = {}): AgentDecision {
  ...
  if (/^\/(?:help|帮助)(?:\s+|$|[，。！？、,.!?;；:：])/i.test(value)) {
    const helpText = `### 💡 UniSearch 快捷指令与技能指南\n\n`
      ...
      + `- 工具：\`@网页搜索\` / \`@社媒搜索\` / \`@AI搜索\` / \`@岗位搜索\` / \`@无水印解析\` / \`@博主主页采集\`\n`
  ...
  if (CAPABILITY.test(value)) {
    return { action: 'chat', reply: '我可以先和你讨论调研思路，也可以按你的明确要求从小红书、抖音、快手、哔哩哔哩、微博、贴吧、知乎、搜索引擎和 AI 资讯搜索（AI HOT）采集内容，完成后继续做总结、舆情和竞品分析。普通咨询只会直接回答，不会创建采集任务。' };
  }
  if (PLATFORM_CAPABILITY.test(value)) {
    return { action: 'chat', reply: '目前支持小红书、抖音、快手、B站、微博、贴吧、知乎，百度、必应等网页搜索，DeepSeek 等 AI 问答，以及招聘、投诉和 AI 资讯搜索（AI HOT）Connector。你可以指定一个或多个平台；没有指定时，我会请你选择后再自动开始。' };
  }
  ...
  const currentFactPlatforms = inferResearchPlatforms(value);
  const hasExplicitAiHotMention = /(?:AI\s*(?:HOT|资讯搜索|行业资讯|圈动态|热点|热榜|日报)|aihot\.virxact\.com)/i.test(value);
  if (CURRENT_FACT.test(value) && !PERSISTENT_RESEARCH.test(value)
    && (!currentFactPlatforms.length || (currentFactPlatforms.length === 1 && currentFactPlatforms[0] === 'aihot' && !hasExplicitAiHotMention))) {
    return { action: 'live_answer', reply: '', query: value };
  }
