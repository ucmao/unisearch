...
- 平台类型区别：社交媒体平台（xhs, dy, ks, bili, wb, tieba, zhihu）；搜索引擎（baidu, bing, so360, sogou）；AI 网页问答平台（deepseek, doubao, kimi, nami, qwen, wenxin, yuanbao）。统一显示名依次为 DeepSeek、豆包、Kimi、纳米 AI、通义千问、文心一言、腾讯元宝。当用户提到这些名称或兼容别名时，对应 Connector 自身即为目标平台。不得追问用户“小红书还是微博”等社交平台；只需提取提问关键词即可直接生成包含对应 AI 平台的计划。
