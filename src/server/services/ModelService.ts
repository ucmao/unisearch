...
export class ModelService {
  ...
  async createPlan(
    ...
    const content = await this.chat([
      { role: 'system', content: `你是UniSearch本地情报任务规划器。\n\n${UNISEARCH_PRODUCT_MANUAL}\n\n${platformHelp}\n\n${skillContext ? `用户明确选择的业务 Skill（这是可信的系统配置，优先按其默认范围和分析口径规划）：\n${skillContext}\n\n` : ''}根据完整对话和用户最新目标生成可执行计划。只输出JSON，不要Markdown。字段必须为 goal:string, platforms:string[], capability:"keyword_search"|"content_detail"|"creator_profile"|"comments"|"url_resolve", targets:string[], keywords:string[], connectorOptions:object, contentEnrichment:{mode:"snippet"|"auto"|"full",maxReadItems:number,maxPerDomain:number,concurrency:number,timeoutMsPerUrl:number}, collectionDepth:"quick"|"standard"|"deep", loginType:"qrcode"|"cookie", headless:boolean, analysis:string[], outputs:string[]。platforms只能使用给定代码，至少一个；“搜索引擎”或“所有搜索引擎”对应 ["baidu", "bing", "so360", "sogou", "toutiao"]；“社交平台”对应 ["xhs", "douyin", "kuaishou", "bili", "weibo", "tieba", "zhihu"]；“BOSS 直聘”或 zhipin.com 对应 ["boss"]；“招聘平台”对应 ["boss", "zhaopin", "job51", "liepin"]；“智联招聘”对应 ["zhaopin"]；“黑猫投诉”对应 ["heimao"]；“综合解析”或“媒体解析”对应 ["media_parser"]；“DeepSeek”对应 ["deepseek"]；“Kimi”或“Kimi AI”对应 ["kimi"]；“豆包”或“Doubao”对应 ["doubao"]；“AI搜索”或“AI问答”对应 ["deepseek", "kimi", "doubao", "qwen", "yuanbao", "nami", "wenxin"]。关键词搜索时 keywords 至少一个。搜索引擎任务默认 contentEnrichment.mode="auto"；用户明确只要摘要时用 "snippet"，明确要求全文或正文时用 "full"。maxReadItems 是全部搜索源去重后的正文读取总量，不是每个平台数量；未明确数量时 quick/standard/deep 分别建议 8/16/30。非网页搜索任务固定使用 snippet 和 0。用户明确说“两个关键词”“3个关键词”等数量时，必须把随后列出的每个关键词拆成 keywords 数组中的独立元素，不得合并成一个字符串。若用户在对话中给出了负向排除指令（如“不要采集黑猫投诉”、“不要某平台”、“排除某平台”或“除某平台外”），生成计划时必须在 platforms 中严格剔除被排除的平台，绝不能包含用户明确指定排除的平台。${depthPromptGuide()}详情、主页、评论、URL解析时 targets 必须包含用户给出的 ID 或链接；connectorOptions 按平台代码保存平台专属参数。analysis 不是执行采集的必填项：只有完整对话中明确出现口碑、负面反馈、竞品对比、价格等分析目的时，才提炼为1到3个简要维度；用户只要求搜索或采集时输出空数组，不要自动套用通用分析模板。当前合并后的任务表达为：${JSON.stringify(userText)}` },
      {
        role: 'system',
        content: '平台别名补充（若与前文旧枚举冲突，以本条为准）：arXiv/论文库对应 ["arxiv"]；GitHub/GitHub仓库/GitHub趋势对应 ["github_repositories"]，AI GitHub 趋势设置 connectorOptions.github_repositories.mode="ai"；RSS/RSS新闻/Atom/订阅源对应 ["rss_news"]；AI HOT/AI热点/AI热榜对应 ["aihot"]，热点请求设置 connectorOptions.aihot.content_mode="hot_topics"，日报请求设置为 "latest_daily"；腾讯元宝/元宝对应 ["yuanbao"]；纳米 AI/纳米AI搜索/纳米搜索对应 ["nami"]；文心/文心一言/文心言/文小言对应 ["wenxin"]；BOSS直聘/BOSS 直聘/zhipin.com 对应 ["boss"]，只有求职、招聘平台或官网域名等明确语境中的单独 boss 才作为别名，普通英文 Boss（老板、上司、标题或作品名）不得映射到平台；智联招聘/智联对应 ["zhaopin"]；前程无忧/51job对应 ["job51"]；猎聘/猎聘网/liepin对应 ["liepin"]；招聘平台/招聘网站对应 ["boss", "zhaopin", "job51", "liepin"]；黑猫投诉/黑猫对应 ["heimao"]；综合解析/媒体解析/链接解析对应 ["media_parser"]；网页阅读器/通用网页解析/网页正文/正文提取对应 ["web_reader"]；AI搜索/AI问答对应 ["deepseek","kimi","doubao","qwen","yuanbao","nami","wenxin"]。AI HOT 的热点、日报、GitHub 趋势以及 RSS 最新新闻允许 keywords 为空。',
      },
      ...messages,
