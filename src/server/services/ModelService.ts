...
export class ModelService {
  ...
  async createPlan(
    ...
    try { return parseModelJson<ResearchPlan>(content); }
      ...
      try {
        return await this.repairJson<ResearchPlan>(content, 'ResearchPlan 对象，包含 goal、platforms、capability、targets、keywords、connectorOptions、collectionDepth、loginType、headless、analysis、outputs', signal);
      } catch {
...
3. create_plan/revise_plan 必须输出完整 plan JSON 对象；其他动作的 plan 为 null。当用户提到“搜索引擎”或“所有搜索引擎”时对应 ["baidu", "bing", "so360", "sogou"]；当用户提到“社交平台”时对应 ["xhs", "douyin", "kuaishou", "bili", "weibo", "tieba", "zhihu"]；当用户提到“DeepSeek”时对应 ["deepseek"]；当用户提到“Kimi”或“Kimi AI”时对应 ["kimi"]；当用户提到“豆包”或“Doubao”时对应 ["doubao"]；当用户提到“千问”、“通义千问”或“Qwen”时对应 ["qwen"]；当用户提到“元宝”或“腾讯元宝”时对应 ["yuanbao"]；当用户提到“纳米 AI”、“纳米AI搜索”或“纳米搜索”时对应 ["nami"]；当用户提到“文心”、“文心一言”或“文小言”时对应 ["wenxin"]；当用户提到“AI搜索”或“AI问答”时对应 ["deepseek", "kimi", "doubao", "qwen", "yuanbao", "nami", "wenxin"]。采集深度只输出 collectionDepth 一个字段，条数、评论开关、起始页由后端自动推导：用户提到“前三页”、“前 3 页”、“只抓前几页”、“不要评论”或未提评论时设为 quick；“前5页”或只说“开启评论”设为 standard；“前10页”或明确要求子评论、回复评论时设为 deep。修改计划 (revise_plan) 时必须基于 currentPlan 进行增量修改，不得将 plan 置为空或省略。
