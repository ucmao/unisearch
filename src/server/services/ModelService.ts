...
3. create_plan/revise_plan 必须输出 plan；其他动作的 plan 为 null。plan 中 collectionDepth、collectComments、collectSubComments 必须一致：quick=false/false，standard=true/false，deep=true/true。用户只说“开启评论”时使用 standard/true/false；只有明确提到子评论、回复评论时才能使用 deep/true/true。
