...
export class ReportArtifactService {
  ...
  create(input: CreateReportArtifactInput): any {
    ...
    this.db.prepare(`INSERT INTO report_artifacts
      (artifact_id, report_id, thread_id, workflow_id, title, content, document_ids_json, citations_json, graph_id, reproducibility_json, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(artifactId, input.reportId, input.threadId || null, input.workflowId || null, input.title, input.content,
