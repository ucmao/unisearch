import { z } from 'zod';
import { artifactSchema } from '../documents/types';
import { canonicalDocumentSchema } from '../documents/canonical';

export const processorWorkerRequestSchema = z.object({
  schemaVersion: z.literal(1),
  jobId: z.string().min(1),
  processorIds: z.array(z.string().min(1)).min(1),
  documents: z.array(canonicalDocumentSchema).min(1).max(25),
...
export const processorWorkerResultSchema = z.object({
  schemaVersion: z.literal(1),
  jobId: z.string().min(1),
  documents: z.array(canonicalDocumentSchema),
