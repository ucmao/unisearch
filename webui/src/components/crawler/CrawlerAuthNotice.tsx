...
export const CrawlerAuthNotice: React.FC = () => {
  ...
  return (
    <aside className="fixed bottom-5 right-5 z-50 w-[360px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-cyber-border-default bg-cyber-bg-panel/95 shadow-2xl backdrop-blur-xl animate-in slide-in-from-right-4 fade-in duration-200">
      <div className="flex items-center gap-3 border-b border-cyber-border-subtle px-4 py-3">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-amber-500/15 text-amber-400">
          <LogIn className="h-4 w-4" />
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="text-base font-semibold text-cyber-text-primary">采集可能需要你的操作</h3>
        </div>
            ...
            <div key={item.platform} className="overflow-hidden rounded-xl border border-cyber-border-subtle bg-cyber-bg-secondary/70 p-3">
              <div className="flex items-start gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[10px] leading-4 text-cyber-text-muted break-all">{item.reason || '请打开 UniSearch 浏览器确认当前状态'}</p>
                </div>
              </div>
