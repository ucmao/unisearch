...
# Repository: https://github.com/NanmiCoder/MediaRadar/blob/main/database/models.py
...
#
# 本地个人分析模式：creator_hash 字段为兼容既有表结构而保留，当前写入
# 原始平台用户 ID；昵称字段同样保存平台返回的原文，不执行哈希或遮盖。
# 头像、主页链接、签名等未在当前内容/评论表定义的字段仍不会自动落库。
...
class BilibiliVideo(Base):
    ...
    creator_hash = Column(String(255), index=True, comment='创作者原始平台用户ID')
...
class TiebaNote(Base):
    ...
    creator_hash = Column(String(512), index=True, comment='创作者原始平台用户ID或用户链接')
