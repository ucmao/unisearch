...
UniSearch 内置 **32 个开箱即用的专业连接器**，覆盖全网主流社交、搜索、AI 问答及垂直资讯信源：

| 分类 | 包含平台 / 信源 | 核心能力概览 | 运行引擎 |
| :--- | :--- | :--- | :--- |
| 📱 **社交与内容平台** | 小红书 / 抖音 / 快手 / 哔哩哔哩 / 微博 / 知乎 / 百度贴吧 | 关键词搜索、图文视频详情、创作者主页、评论与回复 | Playwright / CDP (支持扫码登录) |
| 🔍 **全网搜索引擎** | 百度 / 必应 / 360 / 搜狗 / 头条 / 神马 / 中国搜索 | 多搜索引擎全网抓取、SERP 排名解析、落地页正文还原 | HTTP 极速并发 / Hybrid 混合引擎 |
| 🤖 **AI 网页问答对比** | DeepSeek / Kimi / 豆包 / 通义千问 / 腾讯元宝 / 纳米AI / 文心一言 | 模拟 LLM Web 问答交互、思考链提取、联网引用溯源 | Playwright 自动化 |
| 📰 **技术与商业资讯** | 36氪 / arXiv 论文 / GitHub 仓库 / AI HOT 热点 | 创投资讯正文、学术论文 API、开源项目元数据、热点榜单 | HTTP API 极速提取 |
| 💼 **垂直招聘与维权** | 智联招聘 / 前程无忧 / 猎聘 / BOSS直聘 / 黑猫投诉 | 岗位 JD 与薪资水位抓取、维权投诉事件与涉诉金额追踪 | Playwright / CDP / HTTP |
| 🛠️ **通用工具与解析** | 通用网页正文阅读器 / 30+ 平台综合无水印解析 | HTTP/HTTPS 正文提取、无水印音视频/图集解析 | HTTP / Hybrid 引擎 |

👉 **[查看 32 个连接器完整功能矩阵与认证文档](docs/connectors-matrix.md)**

---

## 🎯 内置业务技能预设

UniSearch 将多信源采集与 AI 研报分析打包为高阶**业务技能（Business Skills）**，支持斜杠指令一键调度：

- **💡 竞品与商业情报** (`sales-course-intelligence`)：聚合社交平台与搜索引擎，分析竞品定价、营销钩子与用户顾虑，自动生成话术对策。
- **📈 新媒体内容调研** (`marketing-content-research`)：跨小红书、抖音、B站拆解爆款选题，提炼高赞表达方式与评论痛点。
- **🛡️ 品牌 GEO 与风险监测** (`brand-geo-risk-monitor`)：评测品牌在 7 大 AI 模型问答中的可见性与一致性，联动黑猫投诉预警舆情风险。
- **💼 招聘岗位薪酬调研** (`hr-salary-benchmark`)：测算行业薪资水位与城市梯度差异，提取 JD 技能要求与招聘定价参考。

👉 **[查看完整技能规范与 Manifest 结构文档](docs/business-skills/)**
