...
- **子进程隔离沙箱**：爬虫采集（`CrawlerWorker`）和数据处理（`ProcessorWorker`）均运行在独立的 Node.js 子进程中（`child_process.fork`），避免内存泄漏与 CPU 密集计算导致桌面 UI 掉帧卡顿，单个 Worker 崩溃可实现秒级自愈与智能重试。
- **混合多引擎调度**：针对不同信源智能采用最优采集引擎——免认证接口采用 **HTTP 极速并发**、复杂反爬与 SPA 页面采用 **Playwright 无头浏览器**、需要人机验证与风控处理时无缝切换至 **Electron Chromium CDP 桥接**。
- **标准化数据协议**：无论何种平台、何种数据格式，均通过流水线归一化为统一的标准化文档模型，包含标题、正文、发布时间、创作者、多媒体、互动数据、标签及原始元数据。
- **本地优先与隐私安全**：全部采集数据、知识索引与登录态 Cookie 均存储在用户本机 SQLite 和持久化沙箱目录中，绝不上传第三方服务器。
...
- **一键多格式导出**：支持导出为 **Excel、Word、PDF、Markdown、JSON、CSV** 等工业级格式。
