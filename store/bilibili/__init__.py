...
# Repository: https://github.com/NanmiCoder/MediaRadar/blob/main/store/bilibili/__init__.py
...
async def update_bilibili_video(video_item: Dict):
    ...
    save_content_item = {
        ...
        "creator_hash": preserve_user_id(video_user_info.get("mid")),  # 原始平台用户 ID
        "nickname": preserve_nickname(video_user_info.get("name")),  # 原始昵称
...
async def update_bilibili_video_comment(video_id: str, comment_item: Dict):
    ...
    save_comment_item = {
        ...
        "creator_hash": preserve_user_id(user_info.get("mid")),  # 原始平台用户 ID
        "nickname": preserve_nickname(user_info.get("uname")),  # 原始昵称
...
async def update_bilibili_creator_dynamic(creator_info: Dict, dynamic_info: Dict):
    save_dynamic_item = {
        ...
        "creator_hash": preserve_user_id(creator_info.get("id")),  # 原始平台用户 ID
        "user_name": preserve_nickname(creator_info.get("name")),  # 原始用户名称
