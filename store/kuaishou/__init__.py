...
# Repository: https://github.com/NanmiCoder/MediaRadar/blob/main/store/kuaishou/__init__.py
...
async def update_kuaishou_video(video_item: Dict):
    ...
    save_content_item = {
        ...
        "creator_hash": preserve_user_id(user_info.get("id")),  # 原始平台用户 ID
        "nickname": preserve_nickname(user_info.get("name")),  # 原始昵称
...
async def update_ks_video_comment(video_id: str, comment_item: Dict):
    ...
    save_comment_item = {
        ...
        # 原始平台用户 ID：V2: author_id, Old: authorId
        "creator_hash": preserve_user_id(comment_item.get("author_id") or comment_item.get("authorId")),
        # 原始昵称：V2: author_name, Old: authorName
        "nickname": preserve_nickname(comment_item.get("author_name") or comment_item.get("authorName")),
