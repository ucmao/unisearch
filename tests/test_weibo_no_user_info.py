...
def test_weibo_note_preserves_user_identity():
    """note 拍平后的存储 dict 保留原始 user id 与昵称。"""
    import store.weibo as wb
    ...
    # 2. 兼容字段 creator_hash 保存原始 user id
...
def test_weibo_comment_preserves_user_identity():
    """comment 拍平后的存储 dict 保留原始 user id 与昵称。"""
    import store.weibo as wb
    ...
    assert nickname == RAW_COMMENT_NICKNAME
...
def test_weibo_store_end_to_end_sqlite():
    ...
    try:
        ...
        assert crow.creator_hash == str(RAW_COMMENT_USER_ID)
