...
def test_kuaishou_video_preserves_user_identity():
    """video 链路保留原始 user_id 与昵称，未建模字段仍不落库。"""
    ...
    # 兼容字段 creator_hash 保存原始 user_id
...
def test_kuaishou_comment_v2_preserves_user_identity():
    """评论 V2(snake_case)格式保留原始身份字段。"""
    with _patch_create_store() as holder:
...
def test_kuaishou_comment_legacy_preserves_user_identity():
    """评论旧 GraphQL(camelCase)格式保留原始身份字段。"""
    with _patch_create_store() as holder:
