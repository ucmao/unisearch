# -*- coding: utf-8 -*-
"""
本地个人分析模式回归测试：保留原始用户 ID 与昵称，同时继续限制未建模字段。

覆盖：
1. ORM 自省 —— database.models 中无禁用列、creator 档案表已删除、内容/评论表含 creator_hash。
2. 提取层 —— 用 mock API/HTML payload 喂各平台提取器，断言原始用户 ID 与昵称透传。
...
# 当前内容/评论表仍未建模的字段名；用户 ID 通过兼容字段 creator_hash 保存。
FORBIDDEN_KEYS = {
...
def _check_nickname_preserved(d: dict, raw: str, label: str):
    nick_keys = set(d.keys()) & NICK_KEYS
    assert nick_keys, f"[{label}] 未保留任何昵称字段"
    for k in nick_keys:
        val = d[k]
        if val in ("", None):
            continue
        assert val == raw, f"[{label}] {k} 未保留原始昵称: {val}"


def test_identity_helpers_preserve_values():
...
def test_xhs_note_extraction_preserves_user_identity():
    import asyncio
    ...
    _check_nickname_preserved(captured, "小红同学", "xhs_note")


def test_tieba_note_extraction_preserves_user_identity():
    ...
    _check_nickname_preserved(d, "贴吧老哥", "tieba_note")


def test_tieba_comment_extraction_preserves_user_identity():
    from media_platform.tieba.help import TieBaExtractor
    ...
    _check_nickname_preserved(d, "评论员", "tieba_comment")


def test_zhihu_comment_extraction_preserves_user_identity():
    ...
    assert d["creator_hash"] == "z9"
    _check_nickname_preserved(d, "知乎答主", "zhihu_comment")


def test_bilibili_video_dict_preserves_user_identity():
    ...
    _check_nickname_preserved(captured, "UP主大人", "bili_video")
